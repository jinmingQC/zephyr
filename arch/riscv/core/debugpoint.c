...
static int hw_slot_owner(unsigned int cpu, int hw_index)
{
...
static void release_cpu_slot(unsigned int cpu, int slot)
{
...
static bool selected_slot_available(unsigned int cpu, int slot,
				    int hw_index)
{
	int owner = hw_slot_owner(cpu, hw_index);
...
{
	return a <= b + b_size - 1U && b <= a + a_size - 1U;
}
...
static uintptr_t build_tdata1(const struct z_debugpoint *dp,
			      enum riscv_trigger_type trigger_type)
{
	...
	if (triggers_load(dp->type)) {
		value |= TDATA1_LOAD;
	}
	if (triggers_store(dp->type)) {
		value |= TDATA1_STORE;
	}
	if (dp->size > 1U) {
...
static int program_type(struct riscv_trigger_slot *slot,
			const struct z_debugpoint *dp,
			enum riscv_trigger_type trigger_type)
{
	uintptr_t requested = build_tdata1(dp, trigger_type);
	uintptr_t tdata2 = dp->size == 1U ? (uintptr_t)dp->addr :
			   napot_encode((uintptr_t)dp->addr, dp->size);
	uint8_t version = 0U;
	...
	csr_write_imm(CSR_TDATA1, slot->tdata1);
	actual = csr_read_imm(CSR_TDATA1);

	if (atomic_get(&slot->active) == 0) {
		reset_selected_trigger();
		return 0;
	}
	if (!config_matches(actual, slot->tdata1, trigger_type, version) ||
	    csr_read_imm(CSR_TDATA2) != tdata2) {
		reset_selected_trigger();
		return -ENOTSUP;
	}
...
static int program_new_slot(struct riscv_trigger_slot *slot,
			    const struct z_debugpoint *dp,
			    uintptr_t initial_tdata1)
{
	...
	int ret = program_type(slot, dp, first);

	if (ret == 0) {
		return 0;
	}

	return program_type(slot, dp, second);
}

static int program_new_cpu_slot(struct riscv_trigger_slot *slot,
				const struct z_debugpoint *dp,
				unsigned int cpu, int index)
{
	...
	for (int hw_index = 0; hw_index < g_cpu_trigger_count[cpu]; hw_index++) {
		...
		if ((int)csr_read_imm(CSR_TSELECT) != hw_index ||
		    !selected_slot_available(cpu, index, hw_index)) {
			continue;
		}
		...
		ret = program_new_slot(slot, dp, initial_tdata1);
		if (ret == 0) {
			return 0;
		}

		release_cpu_slot(cpu, index);
	}

	return ret;
}

static int try_program_existing_slot(const struct riscv_trigger_slot *slot,
				     unsigned int cpu, int index,
				     int hw_index)
{
	csr_write_imm(CSR_TSELECT, hw_index);
	...
	if (!config_matches(actual, slot->tdata1, slot->trigger_type,
			    slot->tinfo_version) ||
...
static int program_existing_slot(const struct riscv_trigger_slot *slot,
				 unsigned int cpu, int index)
{
	...
	if (mapped >= 0) {
		ret = try_program_existing_slot(slot, cpu, index, mapped);
		if (ret == 0) {
			return 0;
		}
	}
	...
	for (int hw_index = 0; hw_index < g_cpu_trigger_count[cpu]; hw_index++) {
		...
		int candidate_ret =
			try_program_existing_slot(slot, cpu, index, hw_index);
...
{
	if (!is_watchpoint_type(dp->type)) {
		return -ENOTSUP;
	}
	if (!range_is_representable((uintptr_t)dp->addr, dp->size)) {
		return -ENOTSUP;
	}
...
int arch_debugpoint_install_local(const struct z_debugpoint *dp)
{
	...
	for (int i = 0; i < g_slot_count; i++) {
		if (atomic_get(&g_slots[i].active) != 0 &&
		    ranges_overlap((uintptr_t)dp->addr, dp->size,
	...
	ret = program_new_cpu_slot(slot, dp, cpu, index);
	if (ret != 0) {
...
int arch_debugpoint_cpu_sync(void)
{
	...
	for (int i = 0; i < g_slot_count; i++) {
		...
		ret = program_existing_slot(slot, cpu, i);
		if (ret != 0) {
			break;
		}
...
static bool select_cpu_slot(unsigned int cpu, int slot)
{
...
static bool foreign_trigger_enabled(unsigned int cpu)
{
	...
	for (int hw_index = 0;
		...
		if (hw_slot_owner(cpu, hw_index) < 0 &&
		    trigger_may_fire(tdata1)) {
			return true;
		}
	}
...
int z_riscv_debugpoint_handle(struct arch_esf *esf)
{
...
#ifdef CONFIG_RISCV_S_MODE
	__asm__ volatile("csrr %0, stval" : "=r"(access_addr));
#else
	...
	int ebreak_status =
		hit_count == 0 ? instruction_is_ebreak(esf->mepc) : 0;
	bool ebreak = ebreak_status > 0;
	bool instruction_known = ebreak_status >= 0;

	if (hit_count == 0 && !ebreak && instruction_known) {
		for (int i = 0; i < g_slot_count; i++) {
			struct riscv_trigger_slot *slot = &g_slots[i];

			if (!cpu_owns_slot(cpu, i) ||
			    atomic_get(&slot->active) == 0 ||
	...
	if (hit_count == 0) {
		...
		return stale_hit && instruction_known && !ebreak ? 0 : -ENOENT;
	}
