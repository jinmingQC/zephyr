...
static volatile uint32_t g_riscv_concurrent_target __aligned(4);
#endif
...
#if defined(CONFIG_RISCV_ISA_EXT_ZAAMO)
static void verify_riscv_amo_hit(struct k_watchpoint *wp)
{
	bool persistent = g_cb_timing == K_WATCHPOINT_TIMING_AFTER;

	zassert_equal(k_watchpoint_is_active(wp), persistent);
	zassert_equal(g_cb_rearm_required, !persistent);
}

ZTEST(watchpoint, test_riscv_zaamo_word_replay_safe)
{
	...
	zassert_equal(z_test_watchpoint_amoadd32(&g_riscv_amo32, 4U), 3U);
	zassert_equal(atomic_get(&g_cb_count), 1);
	zassert_equal(g_riscv_amo32, 7U);
	verify_riscv_amo_hit(&wp);

	rearm_if_needed(&wp);
	zassert_equal(z_test_watchpoint_amoadd32(&g_riscv_amo32, 4U), 7U);
	zassert_equal(atomic_get(&g_cb_count), 2);
	zassert_equal(g_riscv_amo32, 11U);
	verify_riscv_amo_hit(&wp);
	zassert_ok(k_watchpoint_remove(&wp));
}

#if __riscv_xlen >= 64
ZTEST(watchpoint, test_riscv_zaamo_doubleword_replay_safe)
{
	...
	zassert_equal(z_test_watchpoint_amoadd64(&g_riscv_amo64, 4U), 3U);
	zassert_equal(atomic_get(&g_cb_count), 1);
	zassert_equal(g_riscv_amo64, 7U);
	verify_riscv_amo_hit(&wp);

	rearm_if_needed(&wp);
	zassert_equal(z_test_watchpoint_amoadd64(&g_riscv_amo64, 4U), 7U);
	zassert_equal(atomic_get(&g_cb_count), 2);
...
#if defined(CONFIG_RISCV)
struct smp_scalar_writer_request {
...
static void scalar_writer_fn(void *p1, void *p2, void *p3)
{
	ARG_UNUSED(p2);
	ARG_UNUSED(p3);

	struct smp_scalar_writer_request *request = p1;
	...
	for (int i = 0; i < request->iterations; i++) {
		z_test_watchpoint_store32(request->target, request->value + i);
	}
}
#endif
...
ZTEST(watchpoint, test_riscv_smp_concurrent_scalar_hits)
{
	if (arch_num_cpus() < 2) {
		ztest_test_skip();
	}
	...
	static K_WATCHPOINT_DEFINE(wp, (void *)&g_riscv_concurrent_target, 1U,
				   K_WATCHPOINT_WRITE, count_callback,
				   &hit_count);
	struct k_sem ready;
	struct k_sem start;
	struct smp_scalar_writer_request first = {
		.target = &g_riscv_concurrent_target,
		.ready = &ready,
		.start = &start,
		.iterations = 4,
		.value = 0x100U,
	...
	struct smp_scalar_writer_request second = {
		.target = &g_riscv_concurrent_target,
		.ready = &ready,
		.start = &start,
		.iterations = 4,
	...
	k_tid_t first_tid = k_thread_create(
		...
		scalar_writer_fn, &first, NULL, NULL, K_PRIO_PREEMPT(7),
		0, K_FOREVER);
	k_tid_t second_tid = k_thread_create(
		&g_adder_thread, g_adder_stack, SMP_STACK_SIZE,
		scalar_writer_fn, &second, NULL, NULL, K_PRIO_PREEMPT(7),
		0, K_FOREVER);
