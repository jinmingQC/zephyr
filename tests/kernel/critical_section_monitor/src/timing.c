...
uint32_t __wrap_sys_clock_cycle_get_32(void)
{
	...
	if (test_clocks[cpu].enabled) {
		test_clocks[cpu].reads++;
	}
...
static int capture_early_spin_timing(void)
{
	static struct k_spinlock lock;
	unsigned int key = arch_irq_lock();
	unsigned int cpu = CPU_ID;

	early_timing.ready = z_critical_section_monitor_is_ready();
	set_test_clock(0U);
	test_clocks[cpu].reads = 0U;
	k_spinlock_key_t spin_key = k_spin_lock(&lock);

#if defined(CONFIG_SPIN_LOCK_TIME_LIMIT) && (CONFIG_SPIN_LOCK_TIME_LIMIT != 0)
	early_timing.start = lock.lock_time;
#endif
	set_test_clock(32U);
	k_spin_unlock(&lock, spin_key);
	early_timing.reads = test_clocks[cpu].reads;
	restore_clock();
	arch_irq_unlock(key);
	return 0;
}

SYS_INIT(capture_early_spin_timing, PRE_KERNEL_2, 99);

ZTEST(kernel_critical_section_monitor, test_spin_timing_before_monitor_init)
{
	zassert_false(early_timing.ready);
#if defined(CONFIG_SPIN_LOCK_TIME_LIMIT) && (CONFIG_SPIN_LOCK_TIME_LIMIT != 0)
	zassert_equal(early_timing.reads, 2U, "time-limit sampling must not wait for the monitor");
	zassert_equal(early_timing.start, 0U, "zero is a valid timestamp");
#else
	zassert_equal(early_timing.reads, 0U, "the monitor must not sample before initialization");
#endif
}

ZTEST(kernel_critical_section_monitor, test_spinlock_shared_timestamps)
{
	struct k_spinlock lock = {0};

	for (unsigned int i = 0U; i < 3U; ++i) {
		struct z_critical_section_monitor_stats stats;
		k_spinlock_key_t spin_key;
		unsigned int key = arch_irq_lock();
		unsigned int cpu = CPU_ID;
		int result = 0;
		...
		test_clocks[cpu].reads = 0U;
		if (i == 1U) {
			result = k_spin_trylock(&lock, &spin_key);
		} else {
			spin_key = k_spin_lock(&lock);
		}
#if defined(CONFIG_SPIN_LOCK_TIME_LIMIT) && (CONFIG_SPIN_LOCK_TIME_LIMIT != 0)
		uint32_t start = lock.lock_time;
#endif
		set_test_clock(32U);
		if (i == 2U) {
			k_spin_release(&lock);
		...
		unsigned int reads = test_clocks[cpu].reads;

		(void)z_critical_section_monitor_stats_get(cpu, &stats);
		restore_clock();
		arch_irq_unlock(key);

		/* One hold start/end pair, plus the SMP wait start. */
		zassert_ok(result);
		zassert_equal(reads, IS_ENABLED(CONFIG_SMP) ? 3U : 2U);
		zassert_equal(stats.max[Z_CRITICAL_SECTION_MONITOR_EVENT_SPINLOCK_HOLD].cycles,
			      32U);
#if defined(CONFIG_SPIN_LOCK_TIME_LIMIT) && (CONFIG_SPIN_LOCK_TIME_LIMIT != 0)
		zassert_equal(start, 0U, "zero is a valid timestamp");
#endif
	}
}
...
ZTEST(kernel_critical_section_monitor, test_exact_irq_timing)
{
	...
	z_irq_timing_begin(key, 1U);
	set_test_clock(0U);
	z_irq_timing_begin(inner_key, 2U);
	z_irq_timing_end(inner_key);
	...
	zassert_is_null(wrapped.max[Z_CRITICAL_SECTION_MONITOR_EVENT_IRQ_LOCKED].spinlock);
...
int __wrap_z_pend_curr(struct k_spinlock *lock, k_spinlock_key_t key, _wait_q_t *wait_q,
		       k_timeout_t timeout);
void __real_z_spinlock_timing_released(const struct k_spinlock *lock, uint32_t now);
void __wrap_z_spinlock_timing_released(const struct k_spinlock *lock, uint32_t now);
...
void __wrap_z_spinlock_timing_released(const struct k_spinlock *lock, uint32_t now)
{
	...
	__real_z_spinlock_timing_released(lock, now);
	if (stats != NULL) {
...
static bool valid_snapshot(const struct z_critical_section_monitor_stats *stats)
{
	...
	return record->cycles <= SNAPSHOT_ITERATIONS && record->caller == record->cycles &&
	       record->spinlock == NULL && record->thread == &writer_thread && !record->in_isr;
}
