...
bool z_critical_section_monitor_ready;
EXPORT_SYMBOL(z_critical_section_monitor_ready);
...
static Z_CRITICAL_SECTION_MONITOR_FN int critical_section_monitor_init(void)
{
	...
	z_critical_section_monitor_ready = true;
...
interval_start(const struct k_spinlock *spinlock, uintptr_t caller, uint32_t now)
{
		...
		const struct z_critical_section_monitor_interval *interval)
{
	...
	const struct z_critical_section_monitor_record next = {
		...
		.cycles = cycles,
		/*
		 * Intervals close before IRQ restore or the architecture switch.
		 * Unlike _current, ISR nesting is unchanged by scheduler lock handoff.
		 */
...
Z_CRITICAL_SECTION_MONITOR_HOOK void z_irq_timing_begin(unsigned int key, uintptr_t caller)
{
	...
	if (unlikely(!z_critical_section_monitor_is_ready())) {
		return;
	}

	interval = interval_start(NULL, caller, k_cycle_get_32());
	irq_start(state, &interval);
}

Z_CRITICAL_SECTION_MONITOR_HOOK void z_irq_timing_end(unsigned int key)
{
	struct z_critical_section_monitor_cpu *state;
...
EXPORT_SYMBOL(z_irq_timing_end);

Z_CRITICAL_SECTION_MONITOR_HOOK void z_irq_timing_idle_enter(void)
{
	unsigned int key = arch_irq_lock();
	struct z_critical_section_monitor_cpu *state = current_cpu_state();

	if (likely(z_critical_section_monitor_is_ready())) {
		if (arch_irq_unlocked(key)) {
...
EXPORT_SYMBOL(z_irq_timing_idle_enter);

#ifdef CONFIG_SMP
Z_CRITICAL_SECTION_MONITOR_HOOK void
z_spinlock_timing_attempt(const struct k_spinlock *lock, unsigned int key)
{
	struct z_critical_section_monitor_cpu *state;
	struct z_critical_section_monitor_interval interval;
	...
	interval = interval_start(lock, Z_CRITICAL_SECTION_MONITOR_CALLER(), k_cycle_get_32());
...
EXPORT_SYMBOL(z_spinlock_timing_attempt);
#endif /* CONFIG_SMP */

Z_CRITICAL_SECTION_MONITOR_HOOK void
z_critical_section_monitor_spin_acquired(const struct k_spinlock *lock, unsigned int key,
					 uint32_t now)
{
	struct z_critical_section_monitor_cpu *state;
...
#ifdef CONFIG_SMP
	struct z_critical_section_monitor_interval *pending = &state->pending;
	...
	record_max(state, Z_CRITICAL_SECTION_MONITOR_EVENT_SPINLOCK_WAIT,
		   now - pending->start_cycles, pending);

	pending->start_cycles = now;
	spin_hold_start(state, pending);
#else
	struct z_critical_section_monitor_interval interval =
		interval_start(lock, Z_CRITICAL_SECTION_MONITOR_CALLER(), now);
...
EXPORT_SYMBOL(z_critical_section_monitor_spin_acquired);

#ifdef CONFIG_SMP
Z_CRITICAL_SECTION_MONITOR_HOOK void
z_spinlock_timing_abort(const struct k_spinlock *lock, unsigned int key)
{
	struct z_critical_section_monitor_cpu *state;
...
EXPORT_SYMBOL(z_spinlock_timing_abort);
...
Z_CRITICAL_SECTION_MONITOR_HOOK void
z_spinlock_timing_released(const struct k_spinlock *lock, uint32_t now)
{
	struct z_critical_section_monitor_cpu *state;
...
EXPORT_SYMBOL(z_spinlock_timing_released);

Z_CRITICAL_SECTION_MONITOR_HOOK void
z_spinlock_timing_unlocked(const struct k_spinlock *lock, unsigned int key, uint32_t now)
{
	struct z_critical_section_monitor_cpu *state;
...
EXPORT_SYMBOL(z_spinlock_timing_unlocked);
...
EXPORT_SYMBOL(z_critical_section_monitor_irq_lock);
...
EXPORT_SYMBOL(z_critical_section_monitor_irq_unlock);
#endif /* !CONFIG_SMP */
