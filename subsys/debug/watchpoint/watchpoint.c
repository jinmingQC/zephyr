...
static void watchpoint_callback(const struct z_debugpoint *dp,
...
int k_watchpoint_add(struct k_watchpoint *wp)
{
	...
	struct z_debugpoint dp = {
		...
		.addr = wp->addr,
		.size = wp->size,
		.cb = watchpoint_callback,
		.deactivate = watchpoint_deactivate,
		.arg = wp,
		.owner = wp,
		.handle.slot = -1,
	};

	int ret = z_debugpoint_add(&dp);
...
#if defined(CONFIG_WATCHPOINT_HW)
	int ret = z_debugpoint_remove(wp);
...
bool k_watchpoint_is_active(const struct k_watchpoint *wp)
{
	...
	return state == WATCHPOINT_STATE_ARMED || state == WATCHPOINT_STATE_DISARMING;
}
