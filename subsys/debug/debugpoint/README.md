...
software learns an execute trigger's timing only after it fires. A version 1
slot therefore cannot guarantee the before-timed completion trap used below.
...
| `g_cpu_before_step` | Per-hart state for one before-timed completion step. |
...
All Zephyr-owned triggers remain disabled while callbacks run. This prevents
callback memory accesses from recursively hitting an owned point. Other harts
retain their own mappings and continue monitoring concurrently.
...
#### Before timing and continued monitoring

For before timing, returning to the same PC with the data trigger still armed
would immediately trap again. Permanently disabling it would make the point
one-shot. For a known scalar memory instruction, the backend instead reuses the
same physical slot as a temporary execute trigger at the sequential next PC.

```mermaid
sequenceDiagram
    participant D as Data trigger
    participant H as Exception handler
    participant A as Watched instruction
    participant X as Execute trigger at next PC

    D->>H: before-timed data exception
    H->>H: disable triggers and decode instruction
    H->>X: verify exact, before-timed execute match
    H->>H: invoke callback and mask interrupt sources
    H-->>A: return to original PC
    A->>A: complete load or store
    X->>H: exception before next instruction
    H->>D: restore data triggers and interrupt mask
    H-->>X: return to next PC
```

The handler performs these steps:

1. Fetch and allowlist the interrupted instruction through an exception-fixup
   helper.
2. Compute its sequential two- or four-byte next PC.
3. Dry-run an exact execute trigger at that PC and verify through CSR readback
   that it has before timing.
4. Invoke callbacks while all owned data triggers remain disabled.
5. Mask this hart's `mie` interrupt sources, arm the execute trigger, and
   return to the original PC.
6. Let the memory instruction complete, then take the execute-trigger exception
   before the following instruction runs.
7. Restore every active data trigger and the saved interrupt mask.

Persistent before-timing is allowlisted for:

- standard integer scalar loads and stores;
- standard scalar floating-point loads and stores; and
- supported compressed integer and floating-point loads and stores, including
  `C.LW/SW`, `C.LD/SD`, `C.FLW/FSW`, and `C.FLD/FSD` forms.

Compressed forms are accepted only when their corresponding ISA extensions are
enabled. Unknown, custom, and longer encodings are rejected instead of guessing
their length or restart behavior.

The following cases become one-shot:

| Case | Reason |
| --- | --- |
| Unknown timing | Safe retirement or completion cannot be proven. |
| Atomic memory operation | A trigger can observe separate load and store portions. |
| LR/SC | Exception entry can invalidate the reservation. |
| Vector memory instruction | One instruction can contain several restartable accesses. |
| Unknown or custom instruction | Length and replay behavior are not proven. |
| Version 1 `mcontrol6` | Execute-trigger timing cannot be known before it fires. |
| Execute match rejected by readback | Completion at the next PC is not guaranteed. |
| Instruction fetch or decode failure | A safe sequential next PC is unknown. |
| Unsupported return context | The completion exception is not guaranteed. |

For a one-shot case, the backend disables the logical point before invoking the
callback and sets `rearm_required`. The interrupted instruction resumes with
that data trigger inactive. Application code can add the descriptor again later
from thread context.

If another fault occurs after a completion step starts but before its next-PC
trigger, the regular fault path aborts the step and restores trigger and
interrupt state before processing that fault.

Sdtrig permits a before trigger after an instruction has partially executed.
Replaying an ordinary RAM load or store is normally idempotent, which matches
the intended memory-corruption use case. Side-effecting MMIO must not be watched
with this persistent path. Before-timed atomic, LR/SC, vector, and unknown
instructions must also be treated as diagnostic events whose portable
completion semantics cannot be guaranteed.

Each hart can have at most one completion step in flight. Other harts keep the
watchpoint armed while one hart completes its access.
...
RV32 and RV64. The matrix includes uniprocessor, SMP, userspace, and
...
- Zaamo word/doubleword completion and timing-dependent rearm behavior;
- simultaneous scalar-store hits on two harts;
