...
Spinlock hold timing runs from the end of acquisition validation to just before
unlocking. SMP wait timing includes validation; on UP, IRQ timing starts with
the hold. ``k_spin_unlock()`` ends both measurements at the same sample, excluding
the following unlock, statistics update, and interrupt restore.
``k_spin_release()`` ends only the hold.

:kconfig:option:`CONFIG_SPIN_LOCK_TIME_LIMIT` shares the hold samples but still
checks each ``k_spin_unlock()``, even before monitor initialization or when its
tracking slots are full. ``k_spin_release()`` does not check this limit.
